...
from yolov3_utils.utils import xyxy2xywh
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16,
        ...
        annotations = [k.split(sep) for k in img_files if k]
        if sort_files:
            print('Sorting image files...')
            annotations.sort(key=lambda x: x[1])

        labels = [k[2:] for k in annotations]
        self.img_files = [k[1] for k in annotations]

        self.sequences = list(set([os.path.dirname(k) for k in self.img_files]))

        self.seq_to_n_frames = {
            _seq: len([k for k in self.img_files if k.startswith(_seq)])
        ...
        print('Found {} sequences with {} annotations'.format(self.n_sequences, n_files))

        bi = np.floor(np.arange(n_files) / batch_size).astype(np.int)  # batch index
        nb = bi[-1] + 1  # number of batches
        assert n_files > 0, 'No images found in %s' % path
        ...
        # self.labels = [x.replace('images', 'labels').
        #                         replace('.jpeg', '.txt').
        #                         replace('.jpg', '.txt').
        #                         replace('.bmp', '.txt').
        ...
        if self.rect:
            ...
            if os.path.exists(sp):  # read existing shapefile
                ...
                assert len(s) == n_files, 'Shapefile out of sync, please delete %s and rerun' % sp
        ...
        self.imgs = [None] * n_files
        self.labels = [np.zeros((0, 5))] * n_files
        # iter = tqdm(self.labels, desc='Reading labels') if n_files > 1000 else self.labels
        for i in range(n_files):
            img = self.img_files[i]
            label = labels[i]
            # try:
            if len(label) % 5 != 0:
                raise IOError('Invalid label: {} for img {}: {}'.format(label, i, img))
            n_boxes = int(len(label) / 5)
            l = np.zeros((n_boxes, 5))
            for box_id in range(n_boxes):
                class_id, x_center, y_center, width, height = [float(x) for x in label[box_id*5:box_id*5+5]]
                l[box_id, :] = [class_id, x_center, y_center, width, height]

            assert (l >= 0).all(), 'negative labels: %s' % label
            assert (l[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels: %s' % label
            self.labels[i] = l
            ...
            #     print('Warning: missing labels for {} :: {}'.format(self.img_files[i], e))  # missing label file
...
def letterbox(img, new_shape=416, color=(127.5, 127.5, 127.5), mode='auto'):
    ...
    try:
        ...
        print('new_unpad: {}'.format(new_unpad))
