...
def main():
    ...
    with open(args.data_path) as f:
        ...
        for row in csv_reader:
            print('row', row)
            data2class[row[0].split('/')[-1]] = row[3] #row[1]
            data.append(row[0])
    ...
    for count,item in enumerate(data):
        # TODO: skip items that have no audio files
    ...
    print('GT Array', np.argmax(gt_array, 1))
    print('Predicted Array', np.argmax(pred_array, 1))
    print('Accuracy', np.mean(np.argmax(pred_array, 1) == np.argmax(gt_array, 1)))
